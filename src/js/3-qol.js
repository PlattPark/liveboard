...
(function(){
  ...
  render=function(s){
    ...
    (function(){ var h=document.getElementById('hero'); if(!h) return; var mk=function(c,t){ var e=document.createElement('div'); e.className=c; e.textContent=t; return e; };
      ...
      h.classList.add('on'); })();
    // Sept 22: the 10oz price rides the gold tag again - the bar uses the wall to answer "how much is a 10?". From beers.json price10; no price10, no 10oz.
    (s.beers||[]).forEach(function(b){ if(!b.price10||b.tenOzOnly) return; var u=document.getElementById('u-'+b.id); var tag=u&&u.querySelector('.price .tag'); if(!tag||tag.querySelector('.p10b')) return;
      var abv=tag.querySelector('.abv'), dot=tag.querySelector('.dot'); if(!abv||!dot) return;
      var d2=document.createElement('span'); d2.className='dot'; d2.textContent='\u00b7'; var l=document.createElement('span'); l.className='p10'; l.textContent='10oz'; var p=document.createElement('span'); p.className='p10b'; p.textContent='$'+b.price10.toFixed(2).replace(/\.00$/,'');
      tag.insertBefore(d2,dot); tag.insertBefore(document.createTextNode(' '),dot); tag.insertBefore(l,dot); tag.insertBefore(document.createTextNode(' '),dot); tag.insertBefore(p,dot); tag.insertBefore(document.createTextNode(' '),dot); tag.classList.add('has10'); });
  ...
  function shade(){
    ...
    document.body.style.backgroundColor = night ? '#1c2818' : dusk ? '#e9dfb6' : '';   // inline too: on Sept 22 the wall's browser painted cream paper under cream names - the class alone wasn't enough there
